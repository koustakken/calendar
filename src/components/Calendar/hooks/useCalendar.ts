...
export const useCalendar = ({
  ...
  const calendarDays = React.useMemo(() => {
    ...
    const numberOfPrevDays =
      firstDay.dayNumberOnWeek - 1 - shiftIndex < 0
        ? DAYS_IN_WEEK - (firstWeekDayNumber - firstDay.dayNumberOnWeek)
        : firstDay.dayNumberOnWeek - 1 - shiftIndex;

    const numberOfNextDays =
      DAYS_IN_WEEK - lastDay.dayNumberOnWeek + shiftIndex > 6
        ? DAYS_IN_WEEK - lastDay.dayNumberOnWeek - (DAYS_IN_WEEK - shiftIndex)
        : DAYS_IN_WEEK - lastDay.dayNumberOnWeek + shiftIndex;
